...
**Tech Stack**: TypeScript (ES2022), NPM package
**Architecture**: Shared library pattern - provides common types, interfaces, and schemas consumed by multiple services
...
**Databases**: N/A - This is a schema library with no runtime dependencies
**Message Queues**: N/A - Defines schemas used with message queues (Kafka/RabbitMQ) but doesn't directly interact with them
**External APIs**: N/A - Pure TypeScript type definitions
...
**Note**: This library exports TypeScript types and interfaces that define the structure of order-related events. Services import these schemas to ensure type safety when publishing/consuming events.

**Usage Pattern**:
```typescript
import { /* event schemas */ } from '@atelier/order-events-schemas'
```
...
- Provide type safety for event producers and consumers
- Document event schemas and usage patterns

❌ **NOT Responsible For**:
- Runtime validation of events (consumers should validate)
...
🚫 **Do NOT**:
- Add runtime dependencies (keep this library lightweight)
